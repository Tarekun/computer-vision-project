"""End-to-end entrypoint for the experimental pipeline: detection (shared, unchanged,
from common/detection.py) + classification, wired to the same interface as
coins-recognition/pipeline.py's initialize_ght/evaluate. This package forks
reconfigured/ (see ../reconfigured/README.md) and works through the further backlog
in REVIEW.md, with each accepted/rejected idea logged in EXPERIMENTS.md and the final
architecture + honest numbers written up in SOLUTION.md.
"""
...
from .classify import classify_scene
from .reference import build_reference_model


def initialize(reference_dir):
    """Builds the model (shape atlas + colour prototypes + data-derived offdiag +
    SIFT cache) from the reference_set images."""
...
def process_image(img, model):
    ...
    labels = classify_scene(img, dets, model)
    return [{**d, "pred": lab, "value": C.VALUE[lab]} for d, lab in zip(dets, labels)]
