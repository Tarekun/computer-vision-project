...
CACHE_PATH = Path("/tmp/coins_recognition_experimental_tune_cache.pkl")
...
def cross_validate(cache, offdiag, seed=0, k=4, n_trials=1500, passes=3, log=print):
    """k-fold CV (default k=4, up from reconfigured/tune.py's 2-fold odd/even split):
    fit on k-1 folds, measure on the held-out one, rotated over all k folds. This is
    the honest generalization estimate -- unlike the full-data fit below, these
    held-out numbers were never seen by the search that produced them.

    reconfigured/'s 2-fold CV (71 train / 71 val images per fold) showed real
    overfitting risk: train/val gaps as large as 65% vs. 48% amount accuracy in one
    fold. Moving to k=4 gives each fold ~3x more training data (~107 images) and
    reports the spread across folds explicitly, so a single unlucky split doesn't
    masquerade as a stable estimate (see EXPERIMENTS.md item 7)."""
    def fold_id(item):
    ...
    for i in range(k):
        ...
        best_w, train_m, _ = coordinate_ascent(train, offdiag, rs_w, passes=passes, log=lambda *a: None)
        apply_weights(best_w)
        val_m = evaluate(val, offdiag)
        log(f"CV fold {i}/{k} (train n={train_m['n']}, val n={val_m['n']}): "
            f"train={train_m} val={val_m}")
        fold_results.append({"fold": i, "train_metrics": train_m, "val_metrics": val_m, "weights": best_w})

    val_accs = [f["val_metrics"]["amount_acc"] for f in fold_results]
    mean_val_amount_acc = sum(val_accs) / len(val_accs)
    log(f"cross-validated (held-out) mean amount_acc: {mean_val_amount_acc:.4f} "
        f"(min={min(val_accs):.4f}, max={max(val_accs):.4f})")
