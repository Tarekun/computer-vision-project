...
Forked from reconfigured/appearance.py (itself forked from common/appearance.py
with the aspect-ratio-preserving `crop_disc` fix, kept unchanged here). Adds
`photometric_jitter`, used by reference.py's jittered-atlas construction
(EXPERIMENTS.md item 1) -- everything else is unchanged.
"""
import cv2
import numpy as np
...
def photometric_jitter(bgr, brightness=0.0, contrast=1.0, blur_sigma=0.0):
    """Small photometric perturbation (brightness offset, contrast gain, Gaussian
    blur) of a single crop -- used only to build a jittered ensemble of the ONE
    reference photo per class (EXPERIMENTS.md item 1), never on target images."""
    out = bgr.astype(np.float32) * contrast + brightness
    out = np.clip(out, 0, 255).astype(np.uint8)
    return out if blur_sigma <= 0 else cv2.GaussianBlur(out, (0, 0), blur_sigma)
...
def crop_disc(bgr, cx, cy, r):
    ...
    scale, S2.3). Border-clipped coins are padded by edge replication instead of being
    resized from a clipped, non-square patch -- that used to stretch the coin's aspect
    ratio; this keeps it intact."""
    R = int(r)
    h, w = bgr.shape[:2]
    x0, x1 = cx - R, cx + R
    y0, y1 = cy - R, cy + R
    ...
    xc0, xc1 = max(0, x0), min(w, x1)
    yc0, yc1 = max(0, y0), min(h, y1)
    patch = bgr[yc0:yc1, xc0:xc1]
    if patch.size == 0:
        return None
    ...
    return cv2.resize(patch, (C.CANON, C.CANON), interpolation=cv2.INTER_CUBIC)
