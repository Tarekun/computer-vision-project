...
EXPERIMENTS.md item 3 ("size-first two-stage classification"): when
`C.TWO_STAGE_SIZE_FIRST` is on, a coin whose family/bimetal cue is confident enough
(`FAMILY_GATE_CONF`/`FAMILY_GATE_BIM`) is only allowed to take a label from its
resolved family's 2-3 classes during the beam search (`beam_assign_restricted`),
instead of every class -- so a strong colour/bimetal read can't be second-guessed by
a cross-family shape false-match. Ambiguous coins still see all 8 classes, unchanged.
"""
import cv2
import numpy as np
...
from common.classify import zscore, bm_conf, pair_score, abs_anchor, beam_assign

FAMILY_CLASSES = {
    fam: [c for c in C.CLASSES if C.FAMILY[c] == fam]
    for fam in ("copper", "gold", "bimetal")
}


def beam_assign_restricted(unaries, radii, candidate_sets):
    """Like common.classify.beam_assign, but coin i may only take a label from
    candidate_sets[i] instead of every class (EXPERIMENTS.md item 3)."""
    beam = [([], 0.0)]
    for i, unary in enumerate(unaries):
        nxt = []
        for labels, score in beam:
            for lab in candidate_sets[i]:
                p = sum(
...
def fuse_scene(coins, circles, model):
    ...
    for coin in coins:
        ...
        # EXPERIMENTS.md item 6: SIFT_FLOOR keeps the cue partially on even at zero
        # confidence; SIFT_HARD_GATE instead switches it fully off below a threshold.
        if C.SIFT_HARD_GATE:
            sift_norm = 1.0 if coin["sift_conf"] >= C.SIFT_HARD_GATE_THRESHOLD else 0.0
        else:
        ...
        radii.append(coin["r"])
        if C.TWO_STAGE_SIZE_FIRST:
            if bm >= C.FAMILY_GATE_BIM:
                candidate_sets.append(FAMILY_CLASSES["bimetal"])
            elif coin["fam"] and coin["fam_conf"] >= C.FAMILY_GATE_CONF:
                candidate_sets.append(FAMILY_CLASSES[coin["fam"]])
            else:
                candidate_sets.append(C.CLASSES)

    beam_labels = (
        beam_assign_restricted(unaries, radii, candidate_sets)
        if C.TWO_STAGE_SIZE_FIRST else beam_assign(unaries, radii)
    )
    labels, k = [], 0
