...
and the joint beam-search labelling (S6.3-S6.5) -- extended with the SIFT
matching cue (sift_cue.py) this variant adds alongside shape/family/bimetal.

The generic helpers (z-scoring, bimetal confidence, pairwise size scoring, the
absolute bimetal anchor, and the beam search itself) are shared with giacomo
and live in common/classify.py.
"""
import cv2
import numpy as np
...
def classify_scene(img, dets, model):
    ...
    for coin in coins:
        ...
        sift_sc = {c: C.W_SIFT * sift_norm * (2.0 * coin["sift_scores"][c] - 0.25) for c in C.CLASSES}
