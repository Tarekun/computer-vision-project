...
Forked from common/appearance.py with one fix (see ANALYSIS.md S4): `crop_disc`
no longer anisotropically stretches coins that are only partially in-frame.
The original clipped the bounding box asymmetrically at the image border and
force-resized whatever was left to a square canvas, distorting the coin's
aspect ratio before every downstream cue. Here the box is always kept square
(2r x 2r) by replicating border pixels into the missing region, then resized
-- so an off-centre coin's true shape is preserved instead of stretched.
Everything else is unchanged from common/appearance.py.
"""
import cv2
import numpy as np
