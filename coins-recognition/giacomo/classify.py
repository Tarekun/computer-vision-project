...
and the joint beam-search labelling (S6.3-S6.5).

The generic helpers (z-scoring, bimetal confidence, pairwise size scoring, the
absolute bimetal anchor, and the beam search itself) are shared with tiziano
and live in common/classify.py. This file holds only classify_scene, which is
genuinely different between the two packages (tiziano folds in a SIFT cue and
uses its own W_ORDER/W_SAME).
"""
import numpy as np

from . import config as C
from common.appearance import crop_disc, nlm, decast, shape_scores, two_tone_family, first_pass, bimetal_step, ring_sign
    ...
    for coin in coins:
        ...
        template = {
            c: C.TEMPLATE_W * v for c, v in zscore(
                {c: coin["shape"][c] - C.TEMPLATE_ALPHA * C.TEMPLATE_OFFDIAG[c] for c in C.CLASSES}
            ).items()
        }
        ...
        for c in C.CLASSES:
            u = (
                damp * (template[c] + fam_sc[c]) + bim[c]
